      ...
      <header className="mb-6">
        <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4">
                ...
                {basics.title ? (
                    <div className="text-muted-foreground mt-1">{basics.title}</div>
                ) : null}
                </div>
            </div>
          ...
          {data.experience.map((x) => {
            ...
            return (
              <div key={x.id}>
                ...
                {x.location ? (
                  <div className="text-xs text-muted-foreground mt-0.5">{x.location}</div>
                ) : null}
              ...
              <div key={x.id}>
                <div className="flex items-baseline justify-between gap-3">
                  <div className="font-medium">
                    {x.school}
                    {x.degree ? (
                      <span className="text-muted-foreground"> — {x.degree}</span>
                    ) : null}
                  </div>
                  {range ? (
                    <div className="text-xs text-muted-foreground">{range}</div>
                ...
                {x.details ? (
                  <div className="text-sm mt-1 whitespace-pre-wrap">{x.details}</div>
                ) : null}
              </div>
            );
          })}
